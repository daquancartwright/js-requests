...
const ohMy = () => {
    const addQuery = '?name=lion'
    axios.get(`http://localhost:3000/animals${addQuery}`)
    .then(res => {
        // console.log(res.data)
        for (let i = 0; i < res.data.length; i++) {
            const currStr = res.data[i]
            const newElement = document.createElement('p')
            newElement.textContent = currStr
...
const repeatMyParam = () => {
    const someParam = 'DaQuan'
    axios.get(`http://localhost:3000/repeat/${someParam}`)
    .then((res) => {const repeatText = document.getElementById('repeat-text');
...
const queryRequest = () => {
    const queryParam = '?name=DaQuan'

    axios.get(`http://localhost:3000/query-test?query=${queryParam}`)
    .then(res => {
        console.log(res.data)
    })
    .catch(error => {
